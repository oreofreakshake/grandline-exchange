...
import { hasEnoughBerries, updateBerriesBalance } from "./users";

//users holding for a character yna mean
async function getOrCreateHolding(
  user_id: string,
  character_id: string,
): Promise<Holding> {
...
export async function buyShares(
  ...
  if (shares < config.min_trade_shares)
    throw new Error(`Minimum shares to buy is ${config.min_trade_shares}`);

  if (shares > config.max_trade_shares)
    throw new Error(`Maximum shares to buy is ${config.max_trade_shares}`);
...
export async function sellShares(
  ...
  if (shares < config.min_trade_shares)
    throw new Error(`Minimum shares to sell is ${config.min_trade_shares}`);
