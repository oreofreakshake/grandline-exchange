import { Character, Holding, PricePoint, TradeType, DEFAULT_MARKET_CONFIG } from "@/types/glx_types";
...
export function calculateNewPrice(
    ...
    const base = Number(character.base_price);
    const vol = Number(character.volatility);
...
export function calculatePriceChange(last: number, prev: number): {diff: number; pct: number} {
...
export function computePosition({character, holdings, avg_buy_price}: PortfolioMathParams) {
    ...
    const profit_loss_percentage = invested === 0 ? 0 : round((profit_loss / invested) * 100);
...
export function computePortfolioSummary(
    ...
    const total_current_value = round(positions.reduce((acc, p) => acc + p.current_value, 0));
    const total_invested = round(positions.reduce((acc, p) => acc + p.invested, 0));

    const total_profit_loss = round(total_current_value - total_invested);
    const total_profit_loss_percentage = total_invested === 0 ? 0 : round((total_profit_loss / total_invested) * 100);
